...
class OffRunSummary:
    ...
    def __init__(self, file_path, location=LST_LOCATION):
        ...
        self.ra_tel = np.asarray(summary["mean_ra"]) * u.rad
        self.dec_tel = np.asarray(summary["mean_dec"]) * u.rad
        self.az_tel = np.asarray(summary["mean_azimuth"]) * u.rad
        self.alt_tel = np.asarray(summary["mean_altitude"]) * u.rad
        self.elapsed_time = np.asarray(summary["elapsed_time"]) * u.s
    ...
    def find_files(self):
        ...
        for obs_id in self.obs_id:
            matching_files = glob.glob(
                f"{self.path_prefix}/dl2_LST-1.Run{obs_id:05d}.h5", recursive=False
            )
